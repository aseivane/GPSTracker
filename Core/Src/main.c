...
void check_buttons()
{
	switch(button_pressed)
	{
		...
		case SEL_BUTTON:
			if(recording)
			{
				statechart_raise_stopRecord(&statechart);
			}
			else
			{
				statechart_raise_startRecord(&statechart);
			}
			break;
	}
	button_pressed = 0;
...
void statechart_recording( Statechart* handle)
{
#ifdef DEBUG
...
void statechart_notrecording( Statechart* handle)
{
#ifdef DEBUG
